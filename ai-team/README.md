...
> **⚠️ リサーチは「オプション」ではなく最重要フェーズ**。ハマりどころはここで見つかる。
> コーディング中に詰まった場合でも、手戻りしてでもリサーチに戻るのが正解（会社リポジトリでの実運用で確認済み）。
>
> 精度を上げるコツ（Claude Code で下書きブラッシュアップ・トークン節約パターン・Gemini への渡し方）は
> [templates/researcher.md の「リサーチプロンプトの精度を上げるコツ」](templates/researcher.md#リサーチプロンプトの精度を上げるコツ実運用知見) を参照。
> 実運用フィードバックの詳細は [docs/knowledge.md](../docs/knowledge.md#リサーチは「オプション」ではなく最重要フェーズ会社運用でわかったこと) にも記載。
...
1. リサーチプロンプトの下書きを書き、**Claude Code にブラッシュアップさせる**（リポジトリ文脈を加味してくれる）
2. ブラッシュアップ後のプロンプトで調査する
3. 結果が長い場合は Gemini に「要点を整理して」と依頼してから次へ
4. 調査結果をテキストで貼り付けて [templates/researcher.md](templates/researcher.md) のフォーマットに整理する
5. 出力を `handoff/issue-XX/YYYYMMDD_research.md` に保存する
6. Step 1 で Gemini PM役に渡す
